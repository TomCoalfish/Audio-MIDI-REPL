...
void SimpleKnob::init(float minVal, float maxVal, float snap, float skew, float initValue, String name)
{
    this->setSliderStyle(Slider::SliderStyle::RotaryVerticalDrag);
    this->setRange(minVal, maxVal, snap);
    this->setValue(initValue);
    this->setTextBoxStyle(TextEntryBoxPosition::NoTextBox, true, 0, 0);
    this->setSkewFactor(skew);
    this->setLookAndFeel(&lookAndFeel);
    this->name = name;
    this->value.setText((String) initValue, dontSendNotification);
}
